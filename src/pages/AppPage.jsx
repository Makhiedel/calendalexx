...
import "./AppPageStyle.css"; // Fix the path to the correct file

const CalendarComponent = () => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(new Date()); // Track the selected date
  const shiftStartDate = new Date(2025, 6, 2); // Jan 4th
  ...
  const getShiftType = (date) => {
    const daysSinceStart = Math.floor(
      (date - shiftStartDate) / (1000 * 60 * 60 * 24)
    );
    const cycleDay = ((daysSinceStart % 6) + 6) % 6; // Ensure positive modulo
    return cycleDay;
  };
  ...
  const getStatusMessage = (date) => {
    ...
    if (shiftType === 0 || shiftType === 1) {
      return dayShiftMessages[
        Math.floor(Math.random() * dayShiftMessages.length)
      ];
    } else if (shiftType === 2 || shiftType === 3) {
      return nightShiftMessages[
        Math.floor(Math.random() * nightShiftMessages.length)
      ];
    } else {
      return freeDayMessages[
        Math.floor(Math.random() * freeDayMessages.length)
      ];
    }
  };
  ...
  return (
    <div className="calendar-container">
      ...
      <div className="status">
        <h3>{getStatusMessage(selectedDate)}</h3>{" "}
        {/* Display the status for the selected date */}
      </div>
